...
public class Version implements Comparable<Version> {
    ...
    public Integer getMajorVersion() {
        return this.version.equalsIgnoreCase("cloud") ? new Integer(Integer.MAX_VALUE) : this.parseVersion(MAJOR_VERSION_OFFSET);
    }

    public Integer getMinorVersion() {
        return this.version.equalsIgnoreCase("cloud") ? new Integer(Integer.MAX_VALUE) : this.parseVersion(MINOR_VERSION_OFFSET);
    }
     ...
     * Build Numbers at the end or don't follow the syntax strictly, e.g. 8.8,
     * 8.3.3.412333, 8.8.0-19675011
     * 
    ...
    public static int compareSemanticVersions(String versionA, String versionB) {
        ...
        for (int i = 0; i < Math.min(versionASplit.length, versionBSplit.length); i++) {
            String versionAItem = versionASplit[i];
            String versionBItem = versionBSplit[i];
            // if the build number contains a dash then replace it with an empty string to
            // avoid number format exception
            versionAItem = versionAItem.contains(SNAPSHOT_DELIMITER) ? versionAItem.replace(SNAPSHOT_DELIMITER, "") : versionAItem;
            int versionANumber = Integer.parseInt(versionAItem);
            int versionBNumber = Integer.parseInt(versionBItem);
